...
# ANSI color helper (no "light/dark" variants; using standard bold codes)
RESET = "\033[0m"
BOLD = "\033[1m"
RED = "\033[1;31m"
GREEN = "\033[1;32m"
...
def pretty_print(p):
    print(f"{BOLD}{CYAN}Name:{RESET} {BOLD}{p['name'].title()}{RESET}")
    print(f"{BOLD}{YELLOW}Height:{RESET} {p['height']}")
    print(f"{BOLD}{YELLOW}Weight:{RESET} {p['weight']}")
    print(f"{BOLD}{MAGENTA}Types:{RESET} {', '.join(p['types']) if p['types'] else 'N/A'}")
    print(f"{BOLD}{MAGENTA}Abilities:{RESET} {', '.join(p['abilities']) if p['abilities'] else 'N/A'}")
    print(f"{BOLD}{GREEN}Base Stats:{RESET}")
    for stat, val in p['base_stats'].items():
        print(f"  {BOLD}{stat.title():12}{RESET} {val}")
